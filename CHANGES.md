...
- `to_html` and `to_html_with` now accept optional `?extra_themes:(string * Theme.theme) list` for labelled extra themes
- When `~theme` is omitted, the first entry in `~extra_themes` becomes the default
