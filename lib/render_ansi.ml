...
let hex_to_rgb hex =
  ...
  if len = 6 then
    ...
    let r = int_of_string ("0x" ^ String.make 1 hex.[0] ^ String.make 1 hex.[0]) in
    let g = int_of_string ("0x" ^ String.make 1 hex.[1] ^ String.make 1 hex.[1]) in
    let b = int_of_string ("0x" ^ String.make 1 hex.[2] ^ String.make 1 hex.[2]) in
    (r, g, b)
  else
    (0, 0, 0)
...
let font_style_to_ansi = function
  | Bold -> bold
  | Italic -> italic
  | Underline -> underline
...
let render_token token =
  ...
  if codes = [] then token.text
  else String.concat "" codes ^ token.text ^ reset
