let check_out_of_bounds regs idx =
  ( try
      ignore (Oniguruma.Region.capture_beg regs idx);
      assert false
    with Invalid_argument _ -> ()
  );
  try
    ignore (Oniguruma.Region.capture_end regs idx);
    assert false
  with Invalid_argument _ -> ()
...
let check_against regs exp_regs =
  ...
  let rec loop i num_regs exp_regs =
    match (num_regs, exp_regs) with
    | 0, [] ->
        ()
    | 0, _ :: _ ->
        assert false
