...
let tokenize_with_ocaml (case : case) ~test_root =
  let grammar_path =
    ...
    else case.grammar_path
  in
  let grammar =
    match case.grammar_format with
    | Json -> TmLanguage.of_yojson_exn (Yojson.Basic.from_file grammar_path)
    | Plist ->
...
let run_vscode_tokenizer ~script_path ~request_path =
  ...
  | Unix.WEXITED 0 -> Yojson.Basic.from_string stdout
  | Unix.WEXITED code ->
    failwith
...
let collect_case_mismatches case ocaml_lines vscode_lines =
  let rec check_lines idx lines_a lines_b acc =
    ...
    | ocaml_tokens :: rest_a, vscode_tokens :: rest_b ->
      let acc =
        if ocaml_tokens = vscode_tokens then acc
        else
...
let check_vscode_compat () =
  ...
  Fun.protect
    (fun () ->
      ...
      let vscode_json =
        run_vscode_tokenizer ~script_path:node_script
